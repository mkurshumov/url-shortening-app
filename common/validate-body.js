...
module.exports = validateBody = (req, res, next) => {
  ...
  if (!url) {
    return next(error(400, "Body must have 'url' property."));
  }
  ...
  if (Array.isArray(url)) {
    if (!url.length) {
      return next(error(400, "URL array is empty."));
    }

    const areUrlsValid = url.every((u) => isValidURL(u));
    if (!areUrlsValid) {
      return next(error(400, "All URLs must be valid."));
    }
  }

  if (!isValidURL(url)) {
    return next(error(400, "URL is invalid."));
  }
